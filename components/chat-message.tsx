...
export function ChatMessage({ message }: { message: Message }) {
  ...
  return (
    <div
      className={`flex items-start gap-4 p-4 ${message.role === "user" ? "bg-zinc-50 dark:bg-zinc-900" : "bg-white dark:bg-zinc-800"}`}
    >
      ...
      <div className="flex-1 space-y-2">
        ...
        <div className="prose dark:prose-invert max-w-none">{renderMessageContent(message.content as string)}</div>
      </div>
    </div>
  )
}
